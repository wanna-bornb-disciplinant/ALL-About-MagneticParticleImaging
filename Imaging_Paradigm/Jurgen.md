...
| Magnetic particle imaging using a field free line | 2007 | JOURNAL OF PHYSICS D: APPLIED PHYSICS 2007 | Juergen Weizenecker，Bernhard Gleich | 最早使用FFL | 2007.1 | 
...
* **#2007.1**  
  一句话概括一下这篇文章，**最早使用FFL进行MPI成像的文章，无真实实验，主要聚焦于FFL的线圈拓扑设计**。

  作者通过比较MRI得到了一个很重要的结论：**同时采集n个voxels的信号会比单独采集提升
  $\sqrt{n}$倍的SNR，原因是在单位采集时间内，信号中的噪声与信号的特征无关，信号的特征就包括spatial extension**。基于上述结论，因此可以考虑MPI的演化过程：最早通过调制场限制成像FFP极小，之后出于信噪比等的考虑使用驱动场+系统矩阵的方式成像，既然都已经这样了，不如再进一步，再次扩大单次扫描的成像区域？

  仿真实验中的一些细节：信号中设置了符合人体的噪声模型、磁化使用朗之万模型、图像重建使用零阶正则化、系统矩阵中未使用噪声(原因是相比测量数据，校准测量时间更短，粒子浓度更小，因此信噪比更高)

  文章的重点在于如何设计符合作者心目中理想的FFL区域并将这个FFL可以旋转移动的磁场，总结如下：**selection field不再静止，selection field unit由32个排布在一个直径为1m的圆上的线圈组成，线圈中心对称轴均指向bore center，线圈内部由正弦平方电流信号激励，最终能产生2.5T/m的梯度场生成FFL，FFL在整体线圈磁场的带动下，每3.88ms旋转一圈；在生成梯度场线圈的外部，有巨大的双向驱动场线圈，驱动场线圈生成的磁场始终垂直于FFL，这使得FFL在旋转的同时前后移动，相较于驱动场的驱动效果，旋转显得更为缓慢，每一次完整的旋转周期，FFL会进行98次的往返振荡**，最终设计如下：

  ![../pics/42.png](../pics/42.png)

  上图要加上一些说明，右上角的轨迹图其实展现的是类似在整个FFL中取定中间一点的轨迹，文章中将这个轨迹称为"the vector of the drive field"，根据旋转和平移的相对速率，这样的线圈拓扑产生的中心点移动轨迹就会出现类玫瑰花(如上)和放射状(平移速度更快)两种。

  作者在相同的仿真条件下，对比了FFP和FFL的成像质量，FFL图像拥有更高的灵敏度和空间分辨率，作者对于FFL的应用的认知是**扩大了无磁场空间的维度，但他自己也认为这个改进不具备进一步的拓展性**。
